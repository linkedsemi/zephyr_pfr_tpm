...
LOG_MODULE_REGISTER(spi_monitor_clk_freq_interrupt, LOG_LEVEL_INF);
...
void spi_monitor_clk_freq_check(const struct device *const spifiltermaster,const struct device *const spifilter)
{

	uint8_t send_freq[]={10};
	...
	for (int i=0; i<ARRAY_SIZE(send_freq);i++) {
		uint32_t freq =send_freq[i]*1000000;
...
int main(void)
{
	...
	spif_clk_check_config(spifilter,0,6,30,1);
	spi_monitor_clk_freq_check(spifiltermaster,spifilter);
	uint16_t act_freq=spif_clk_check_peek(spifilter);
	LOG_INF("act_freq: 0x%04x", act_freq);
	LOG_INF("done");
